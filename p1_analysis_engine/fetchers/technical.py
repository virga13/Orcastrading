...
# yfinance max lookback per interval (in days)
...
VALID_INTERVALS = list(_INTERVAL_LOOKBACK.keys())


def fetch_technical(ticker: str, interval: str = "1d") -> dict:
    """
    Fetch OHLCV data via yfinance at the requested candlestick interval and
    ...
    if interval not in _INTERVAL_LOOKBACK:
        raise DataFetchError(f"Unsupported interval '{interval}'. Choose from: {VALID_INTERVALS}")

    lookback_days = _INTERVAL_LOOKBACK[interval]
    period = f"{lookback_days}d"
    df = yf.Ticker(ticker).history(period=period, interval=interval)

    if df.empty or len(df) < 20:
        raise DataFetchError(f"Insufficient price data for {ticker} (got {len(df)} bars)")
